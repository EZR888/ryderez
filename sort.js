...
document.getElementById('fileInput').addEventListener('change', function (event) {
    ...
    function readFile(index) {
        ...
        reader.onload = async function (e) {
            ...
            let processedRows = [];
            for (let i = 4; i < rows.length; i++) { // Start from row 4 (index 3)
                if (!rows[i].trim()) break;

                let rowValues = rows[i].split(",").map(value => value.trim());

                if (rowValues.every(cell => cell === "")) break;

                // ✅ Limit row to first 38 columns
                processedRows.push(rowValues.slice(0, 37).map(value => (value === "" ? "" : Number(value) || value)));
            }
