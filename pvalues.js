    ...
    function readFile(index) {
        ...
        reader.onload = async function (e) {
            ...
            for (let i = 4; i < rows.length; i++) { // Start from the fourth row (index 3)
                if (!rows[i].trim()) { // Stop at the first completely empty row
                    break;
                }

                let rowValues = rows[i].split(",").map(value => value.trim()); // Trim each cell value

                if (rowValues.every(cell => cell === "")) { // Stop if all cells are empty
                    break;
                }
